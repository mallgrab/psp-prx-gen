...
bool CProcessElf::FillSection(ElfSection& elfSect, const Elf32_Shdr *pSection)
{
	...
	unsigned char* elf_section_size = elfSect.pData + elfSect.iSize;
	unsigned char* elf_size = m_pElf + m_iElfSize;

	bool big_small = elf_section_size > elf_size;
	int total_section_sizes =+ elfSect.iSize;
	int test_size = m_iElfSize - total_section_sizes;
...
bool CProcessElf::LoadSections()
{
	...
	if((m_elfHeader.iShoff != 0) && (m_elfHeader.iShnum > 0) && (m_elfHeader.iShentsize > 0))
	{
		...
		if(m_pElfSections != NULL)
		{
            ...
            // m_pElfSections is empty so we allocate iShnum amount of section headers.
            // we then fill each section header with information (end of the elf = m_elfHeader.iShoff)
			for(iLoop = 0; iLoop < m_iSHCount; iLoop++)
			{
                ...
                // advance to the next section header by adding the section size to the pointer
